...
function attachSocketListeners(socket: Socket) {
  ...
  socket.on("ticket-message", (payload: { ticketId: string; message: LiveSupportMessage }) => {
    broadcast("onMessage", payload.message);
  });
  socket.on("ticket-internal", (payload: { ticketId: string; message: LiveSupportMessage }) => {
