...
export class SanitizePipe implements PipeTransform {
  ...
  transform(value: any, metadata: ArgumentMetadata) {
    ...
    if (typeof value === 'string') {
      ...
      // 🔒 Log if something changed (development only — avoid logging token PII)
      if (
        process.env.NODE_ENV === 'development' &&
        value !== sanitized &&
        (isToken || looksLikeToken)
  ...
  private sanitizeString(str: string, fieldName?: string): string {
    ...
    // 🔒 XSS sanitization only. SQL injection is handled by Prisma's
    // parameterized queries — regex "SQL keyword" stripping here provided no
    // real protection and silently corrupted legitimate content (names, form
    // answers, messages containing words like "select"/"and"/"or"/";"/"--").
    return (
      str
        // إزالة HTML tags
        .replace(/<[^>]*>/g, '')
