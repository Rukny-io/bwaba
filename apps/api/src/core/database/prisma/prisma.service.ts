  ...
  implements OnModuleInit, OnModuleDestroy
{
  ...
  constructor() {
    ...
    // 🔒 F-10: Transparent field-level encryption (gated, default OFF).
    // Enable only after blind-index columns exist AND existing rows are
    // backfilled (see apps/api/scripts/migrate-encrypt-pii.ts).
    if (process.env.FIELD_ENCRYPTION_ENABLED === 'true') {
      try {
        // Lazy require so the app boots even if keys are absent when disabled.
        const {
          EnvKeyProvider,
          createEncryptionMiddleware,
          // eslint-disable-next-line @typescript-eslint/no-var-requires
        } = require('@rukny/database');
        const keys = new EnvKeyProvider();
        // `$extends` extension for Prisma 7+ compatibility; wraps the legacy middleware.
        const middleware = createEncryptionMiddleware(keys);
        const extendedClient = this.$extends({
          query: {
            $allModels: {
              async $allOperations({ model, operation, args, query }) {
                return middleware({ model, action: operation, args }, () => query(args));
              },
            },
          },
        });
        ...
        this.logger.error(
          `Failed to enable field encryption: ${(err as Error).message}`,
        );
        throw err;
      }
    }
