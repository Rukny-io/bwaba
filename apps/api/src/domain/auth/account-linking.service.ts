...
const PROVIDER_ID_FIELDS: Record<
  OAuthProvider,
  'googleId' | 'githubId' | 'linkedinId' | 'facebookId'
> = {
  google: 'googleId',
  github: 'githubId',
  linkedin: 'linkedinId',
...
export interface LinkedProvidersResult {
  ...
  github: { linked: boolean; email?: string };
...
export class AccountLinkingService {
  ...
  async getLinkedProviders(userId: string): Promise<LinkedProvidersResult> {
    ...
    return {
      ...
      github: {
        ...
        email: user.githubId ? user.email : undefined,
      },
      linkedin: {
  ...
  async initiateLinking(
    ...
    const user = await this.prisma.user.findUnique({
      where: { id: userId },
      select: { googleId: true, githubId: true, linkedinId: true, facebookId: true },
    });
