...
export class QuickSignService {
  ...
  async generateQuickSign(
    ...
    let existingUser = await this.redis.get<{
      id: string;
      email: string;
    ...
    const type: QuickSignType = existingUser
      ? QuickSignType.LOGIN
      : QuickSignType.SIGNUP;
    ...
    const jwtToken = this.jwtService.sign(payload, {
      expiresIn: this.QUICKSIGN_EXPIRY_MINUTES * 60,
    ...
    expiresAt.setMinutes(
      expiresAt.getMinutes() + this.QUICKSIGN_EXPIRY_MINUTES,
    );
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(
        '[QuickSign.generateQuickSign] Generated new QuickSign token:',
        {
          email,
          type,
    ...
    await this.redis.set(
      ...
      this.QUICKSIGN_EXPIRY_MINUTES * 60,
    );
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(
          '[QuickSign.generateQuickSign] ✅ Token saved to Database with ID:',
          createdRecord.id,
        );
      }
    } catch (err) {
  ...
  async verifyQuickSign(
    ...
    try {
      ...
      if (!skipLock) {
        ...
        if (!lockAcquired) {
          // 🔒 Don't return used:true here — the token may not actually be used.
          // Return a specific error so the caller can distinguish lock contention
          // from an actually-used token.
          return { valid: false, error: 'locked' } as any;
        }
      }
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(
            '[QuickSign.verifyQuickSign] JWT verification successful:',
            {
              email: payload.email,
              type: payload.type,
              uuid: payload.uuid,
        ...
        if (cachedData) {
          if (!isProduction) {
            console.log(
              '[QuickSign.verifyQuickSign] Token found in Redis cache',
            );
          }
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log(
              '[QuickSign.verifyQuickSign] Token verified from Redis:',
              {
                email: cachedData.email,
        ...
        if (!isProduction) {
          console.log(
            '[QuickSign.verifyQuickSign] Token NOT in Redis, checking database with hash:',
            {
        ...
        if (!quickSign) {
          console.warn(
            '[QuickSign.verifyQuickSign] ❌ Token NOT found in database!',
            {
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log(
              '[QuickSign.verifyQuickSign] Tokens in DB for this email:',
              {
                email: payload.email,
        ...
        const profileCompleted =
          (quickSign as any).users?.profileCompleted || false;
        ...
        if (errorName === 'TokenExpiredError') {
          try {
            ...
            console.log(
              '[QuickSign.verifyQuickSign] Token expired but decoded for email:',
              decoded?.email,
            );
            return {
              valid: false,
        ...
        if (errorName === 'JsonWebTokenError') {
          console.error(
            '[QuickSign.verifyQuickSign] Invalid JWT signature or format',
          );
          return { valid: false };
        }
  ...
  async verifySignupToken(token: string): Promise<{
    ...
    try {
      ...
      console.warn(
        `[QuickSign] Signup token verification failed: ${errorName} - ${errorMessage}`,
      );
