...
export class TokenService {
  // 🔒 إعدادات الأمان (قابلة للضبط عبر .env)
  private readonly ACCESS_TOKEN_EXPIRY: string;
  private readonly ACCESS_TOKEN_SECONDS: number;
  ...
  constructor(
    ...
    this.ACCESS_TOKEN_EXPIRY =
      this.configService.get<string>('ACCESS_TOKEN_EXPIRES_IN') || '30m';
    this.ACCESS_TOKEN_SECONDS = parseDurationToSeconds(
      this.ACCESS_TOKEN_EXPIRY,
      30 * 60,
    );
    this.REFRESH_TOKEN_EXPIRY_DAYS = parseDurationToDays(
  ...
  async generateTokenPair(
    ...
    const accessToken = this.jwtService.sign(accessPayload, {
      expiresIn: this.ACCESS_TOKEN_SECONDS,
      algorithm: 'HS256',
    });
    ...
    const sessionExpiresAt = new Date(
      Date.now() + this.ACCESS_TOKEN_SECONDS * 1000,
    );
