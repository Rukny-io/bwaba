...
export class AppsService {
  ...
  /** إرسال OTP عبر Meta WhatsApp Cloud API فقط. */
  private async sendWhatsAppOtp(
    ...
    try {
      ...
      this.logger.warn(
        `WhatsApp Business OTP failed for app verification: ${(error as Error).message}`,
      );
      return (
        userMessage ||
  ...
  async sendOtp(userId: string, dto: SendAppOtpDto) {
    ...
    if (this.isDevOtpBypass()) {
      this.logger.warn(
        `[DEV] WHATSAPP_OTP_DEV_BYPASS — OTP for ${phoneNumber.slice(0, 4)}***: ${code}`,
      );
    } else {
    ...
    this.logger.log(
      `App verification OTP sent to ${phoneNumber.slice(0, 4)}*** for user ${userId}`,
    );
  ...
  /**
   * ينشئ تطبيقاً جديداً تحت ملكية مساحة العمل (`workspaceOwnerId`)،
   * لكن OTP يُتحقق منه ضد `actorId` (الشخص الفعلي الذي أثبت هاتفه).
   *
   * إذا كان المُنشئ هو المالك، فالقيمتان متطابقتان (السلوك القديم).
   */
  async create(workspaceOwnerId: string, actorId: string, dto: CreateAppDto) {
    await this.checkAppLimit(workspaceOwnerId);
    ...
    this.logger.log(
      `App created: ${app.appId} for workspace ${workspaceOwnerId} by actor ${actorId}`,
    );
