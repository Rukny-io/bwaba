...
export class EmailDomainsController {
  ...
  list(@Req() request: any) { return this.domains.list(request.userId); }

  @Post('domains')
  @RequireScopes('email:domains:write')
  @ApiOperation({ summary: 'Start domain verification' })
  create(@Req() request: any, @Body() dto: CreateEmailDomainDto) { return this.domains.create(request.userId, dto.domain); }

  @Get('domains/:domain')
  @RequireScopes('email:domains:read')
  @ApiOperation({ summary: 'Refresh domain verification status' })
  get(@Req() request: any, @Param('domain') domain: string) { return this.domains.get(request.userId, domain); }

  @Post('senders')
  @RequireScopes('email:domains:write')
  @ApiOperation({ summary: 'Authorize a verified sender for the current app' })
  createSender(@Req() request: any, @Body() dto: CreateEmailSenderDto) {
    if (!request.apiKey?.developerAppId) throw new ForbiddenException('API key is not linked to a developer app.');
    return this.domains.createSender(request.userId, request.apiKey.developerAppId, dto.email);
  }
}
