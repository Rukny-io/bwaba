...
export class EmailDomainsService {
  constructor(private readonly prisma: PrismaService, private readonly ses: MailSesService) {}

  async list(userId: string) {
    const domains = await this.prisma.developerEmailDomain.findMany({
      where: { userId }, orderBy: { createdAt: 'desc' },
      select: { domain: true, status: true, dkimTokens: true, verifiedAt: true, createdAt: true },
    });
  ...
  async create(userId: string, rawDomain: string) {
    ...
    const status = identity.sending ? DeveloperEmailDomainStatus.VERIFIED : DeveloperEmailDomainStatus.PENDING;
    const saved = await this.prisma.developerEmailDomain.upsert({
      where: { userId_domain: { userId, domain } },
      create: { userId, domain, status, dkimTokens: identity.tokens, verifiedAt: identity.sending ? new Date() : undefined },
      update: { status, dkimTokens: identity.tokens, verifiedAt: identity.sending ? new Date() : undefined },
      select: { domain: true, status: true, dkimTokens: true, verifiedAt: true, createdAt: true },
    });
  ...
  async get(userId: string, rawDomain: string) {
    ...
    const record = await this.prisma.developerEmailDomain.findUnique({
      where: { userId_domain: { userId, domain } },
      select: { id: true, domain: true, status: true, dkimTokens: true, verifiedAt: true, createdAt: true },
    });
    ...
    const updated = await this.prisma.developerEmailDomain.update({
      where: { id: record.id },
      data: { status, dkimTokens: identity.tokens, verifiedAt: identity.sending ? record.verifiedAt ?? new Date() : null },
      select: { domain: true, status: true, dkimTokens: true, verifiedAt: true, createdAt: true },
    });
  ...
  async createSender(userId: string, developerAppId: string, rawEmail: string) {
    ...
    if (!ownedDomain) throw new BadRequestException('Domain is not verified for this account.');
    const app = await this.prisma.developerApp.findFirst({ where: { id: developerAppId, userId }, select: { id: true } });
    if (!app) throw new ForbiddenException('Developer app does not belong to this account.');
    const sender = await this.prisma.developerEmailSender.upsert({
      where: { developerAppId_emailDomainId_localPart: { developerAppId, emailDomainId: ownedDomain.id, localPart } },
      create: { developerAppId, emailDomainId: ownedDomain.id, localPart },
      update: { status: 'ACTIVE' },
      select: { id: true, localPart: true, status: true, emailDomain: { select: { domain: true } } },
    });
    return { id: sender.id, email: `${sender.localPart}@${sender.emailDomain.domain}`, status: sender.status.toLowerCase() };
  }
  ...
  private splitAddress(value: string): [string, string] {
    ...
    if (at <= 0 || at === value.length - 1 || /\r|\n/.test(value)) throw new BadRequestException('Invalid sender email.');
    return [value.slice(0, at), value.slice(at + 1)];
  }

  private publicDomain(domain: { domain: string; status: DeveloperEmailDomainStatus; dkimTokens: string[]; verifiedAt: Date | null; createdAt: Date }) {
    return { domain: domain.domain, status: domain.status.toLowerCase(), dkimTokens: domain.dkimTokens, verifiedAt: domain.verifiedAt?.toISOString() ?? null, createdAt: domain.createdAt.toISOString() };
  }
}
