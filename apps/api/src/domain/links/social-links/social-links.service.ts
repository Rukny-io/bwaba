...
export class SocialLinksService extends OwnableService {
  ...
  private async allocateLinkId(): Promise<string> {
    ...
    throw new BadRequestException('تعذر إنشاء معرف الرابط، أعد المحاولة');
  }
  ...
  async create(userId: string, createDto: CreateSocialLinkDto) {
    ...
    if (
      createDto.connectionId &&
      (createDto.layout === 'profile_card' || createDto.layout === 'media_grid')
    ) {
      const conn = await (this.prisma as any).instagramConnection.findFirst({
        where: { id: createDto.connectionId, userId },
        select: { id: true },
      });
      if (!conn) {
        throw new BadRequestException('Invalid Instagram connection');
      }
    }
  ...
  async bulkUpdateStatus(userId: string, dto: BulkUpdateStatusDto) {
    ...
    // 🔒 F-08: per-item ownership enforcement via base class
    await this.assertAllOwned(
    ...
    // Update all links (still scoped by profileId as defense-in-depth)
  ...
  async bulkDelete(userId: string, dto: BulkDeleteDto) {
    ...
    // Delete all links (still scoped by profileId as defense-in-depth)
