...
export class StorageService {
  ...
  private readonly MAX_SUPPORT_ATTACHMENT_SIZE = 5 * 1024 * 1024; // 5MB

  private readonly ALLOWED_SUPPORT_ATTACHMENT_TYPES = [
    'image/jpeg',
    'image/png',
    'image/webp',
    'image/gif',
  ...
  private async validateSupportTicketOwnership(
    userId: string,
    ticketId: string,
  ): Promise<void> {
    ...
    if (!ticket) {
      throw new ForbiddenException('ليس لديك صلاحية الوصول لهذه التذكرة');
    }
  }
  ...
  async requestDirectUpload(
    ...
    const allowedTypes =
      category === FileCategory.SUPPORT_ATTACHMENT
        ? this.ALLOWED_SUPPORT_ATTACHMENT_TYPES
  ...
  private async validateEntityOwnership(
    ...
    switch (category) {
      ...
      case FileCategory.SUPPORT_ATTACHMENT:
        await this.validateSupportTicketOwnership(userId, entityId);
        break;
    }
  }
  ...
  /**
   * Upload a support ticket attachment (images or PDF).
   * Files are stored against the ticket owner's storage quota.
   */
  async uploadSupportAttachment(
    ...
    const storageUserId = ownerUserId ?? uploaderId;
    await this.validateSupportTicketOwnership(storageUserId, ticketId);
    this.validateFileExtension(file?.originalname);
    ...
    if (incomingSize > this.MAX_SUPPORT_ATTACHMENT_SIZE) {
      throw new BadRequestException('حجم الملف يتجاوز 5MB');
    }

    const hasSpace = await this.checkStorageLimit(storageUserId, incomingSize);
    if (!hasSpace) {
      throw new BadRequestException('لا توجد مساحة تخزين كافية');
    }
    ...
    if (
      !fileType ||
      !this.ALLOWED_SUPPORT_ATTACHMENT_TYPES.includes(fileType.mime)
    ) {
      throw new BadRequestException('نوع الملف غير مسموح');
    }

    let uploadBuffer = buffer;
    let uploadMime = fileType.mime;
    let uploadName =
      file.originalname ||
      `attachment.${this.getExtensionFromMime(fileType.mime)}`;

    if (this.ALLOWED_IMAGE_TYPES.includes(fileType.mime)) {
    ...
    const filename = `${uuidv4()}.${this.getExtensionFromMime(uploadMime)}`;
    const key = this.generateKeyForCategory(
      storageUserId,
    ...
    const fileRecord = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.profile.update({
        ...
        data: { storageUsed: { increment: BigInt(uploadBuffer.length) } },
      });
