...
export class UserService {
  ...
  async getProfile(userId: string) {
    ...
    if (result) {
      const phone =
        (result as { phone?: string | null; phoneNumber?: string | null })
          .phone ||
        (result as { phoneNumber?: string | null }).phoneNumber ||
        null;
      (result as { phone: string | null }).phone = phone;
    }
  ...
  async updateProfile(userId: string, updateData: UpdateProfileDto) {
    ...
    if (updateData.phone !== undefined) {
      ...
      const nextPhone = updateData.phone
        ? normalizePhoneNumber(updateData.phone)
        : '';

      if (nextPhone && nextPhone !== currentPhone) {
        throw new BadRequestException(
          'يجب التحقق من رقم الهاتف عبر واتساب قبل الحفظ.',
        );
      }

      if (!nextPhone) {
        updateData.phone = '';
      } else if (nextPhone !== updateData.phone) {
    ...
    const updatedUser = await this.prisma.user.update({
      ...
      data: {
        ...(updateData.phone !== undefined && {
          ...
          phoneNumber: updateData.phone || null,
          ...(!updateData.phone && {
            phoneVerified: false,
