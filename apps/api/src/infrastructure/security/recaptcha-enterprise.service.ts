...
export class RecaptchaEnterpriseService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.siteKey =
      ...
      '6LcYWWAsAAAAAJpv0z4pIQqOIhl05dUUzauUEG2D';
    this.apiKey = this.configService.get<string>('RECAPTCHA_API_KEY') || '';

    if (!this.projectId || !this.apiKey) {
      this.logger.warn(
        'reCAPTCHA Enterprise configuration missing. Service will be disabled.',
      );
    }
  }
  ...
  async verifyToken(
    ...
    if (!this.projectId || !this.apiKey) {
      this.logger.warn(
        'reCAPTCHA verification skipped - service not configured',
      );
      return {
        success: true,
        score: 1.0,
    ...
    try {
      ...
      const valid =
        riskAnalysis.reasons && !riskAnalysis.reasons.includes('INVALID_TOKEN');
      ...
      this.logger.log(
        `reCAPTCHA verification - Score: ${score}, Action: ${action}, Valid: ${valid}, ActionMatch: ${actionMatch}`,
      );
      ...
      if (!isSuccess) {
        ...
        this.logger.warn(
          `reCAPTCHA verification failed - Reasons: ${reasons.join(', ')}, Score: ${score}`,
        );
      }
