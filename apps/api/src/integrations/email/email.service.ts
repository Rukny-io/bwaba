...
export class EmailService {
  ...
  async sendAccountTeamInvitation(
    to: string,
    data: {
      inviterName: string;
      role: string;
    ...
    const accountsUrl = this.configService.get(
      'AUTH_FRONTEND_URL',
    ...
    const html = `
          <div dir="rtl" style="font-family: Arial, sans-serif; max-width: 560px; margin: 0 auto;">
            <h2>دعوة للانضمام لفريق العمل</h2>
            <p>مرحباً،</p>
            <p><strong>${data.inviterName}</strong> دعاك للانضمام إلى فريق <strong>${data.workspaceName}</strong> على ركني بدور <strong>${data.role}</strong>.</p>
            <p>
              <a href="${accountsUrl}/manage/team/invitations" style="display:inline-block;padding:12px 24px;background:#111;color:#fff;text-decoration:none;border-radius:8px;">
                عرض الدعوة
              </a>
            </p>
            <p style="color:#666;font-size:13px;">يجب أن يكون لديك حساب في ركني لقبول الدعوة. إذا لم تتوقع هذه الدعوة، يمكنك تجاهل هذا البريد.</p>
          </div>
        `;
    ...
    try {
      await this.sendEmail({
        to,
        subject: '👥 دعوة للانضمام لفريق العمل على ركني',
        html,
      });
    } catch (error) {
      console.error('Failed to send account team invitation:', error);
    }
  }
