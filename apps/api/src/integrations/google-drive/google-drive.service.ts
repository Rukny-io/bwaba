...
export class GoogleDriveService {
  ...
  /**
   * 🔒 Resolve the HMAC signing secret for signed URLs.
   * Fails closed: throws if JWT_SECRET is missing instead of falling back to a
   * hardcoded default that would make signatures forgeable.
   */
  private getSigningSecret(): string {
    const secret = this.config.get<string>('JWT_SECRET');
    if (!secret) {
      throw new Error(
        'JWT_SECRET is not configured — cannot sign/verify secure file URLs',
      );
    }
    return secret;
  }
