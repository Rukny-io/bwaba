...
export class QasehPaymentService {
  ...
  constructor(private readonly config: ConfigService) {
    ...
    this.webhookSecret = this.config.get<string>('QASEH_WEBHOOK_SECRET', '');
  }

  /**
   * 🔒 Verify the HMAC-SHA256 signature of an incoming Qaseh webhook.
   *
   * When `QASEH_WEBHOOK_SECRET` is configured, the raw request body is verified
   * against the `x-qaseh-signature` header (constant-time comparison). If no
   * secret is configured we log a warning and allow the request through, because
   * the webhook handler already re-fetches authoritative status from Qaseh and
   * never trusts the body for amounts/status.
   */
  verifyWebhookSignature(req: RawBodyRequest<Request>): boolean {
    ...
    const headerSignature = (req.headers?.['x-qaseh-signature'] ||
