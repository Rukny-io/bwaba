...
export class UploadController {
  ...
  private async handleDeleteBanners(
    ...
    const requestedKeys = body.keys || body.items || [];
    if (requestedKeys.length === 0) {
      return { ok: true, deleted: 0 };
    }

    // 🔒 Only allow deletion of keys under the caller's own prefix
    const userId = req.user?.id;
    if (!userId) {
      return { ok: true, deleted: 0 };
    }
    ...
    const keysToDelete = requestedKeys.filter((k) => k.startsWith(ownerPrefix));
    ...
    const result = await this.uploadService.deleteKeys(keysToDelete, userId);
