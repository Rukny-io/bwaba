...
export class UploadService {
   ...
   *
   * 🔒 Ownership scoping: only keys under the caller's own prefix
   * (`users/{userId}/`) are deletable. This prevents an authenticated user from
   * deleting arbitrary S3 objects by supplying another user's key.
   */
  async deleteKeys(keys: string[], userId: string) {
    if (!userId) {
    ...
    const ownerPrefix = `users/${userId}/`;
    const ownedKeys = keys.filter((key) => key.startsWith(ownerPrefix));

    const rejected = keys.length - ownedKeys.length;
    if (rejected > 0) {
      this.logger.warn(
        `deleteKeys: rejected ${rejected} key(s) not owned by user ${userId}`,
      );
    }
