...
function LoginContent() {
  ...
  const nextPath = useMemo(
    () => resolveClientNext(searchParams.get('next'), '/apps'),
    [searchParams],
  );
    ...
    <AuthShell className="max-w-[460px]">
      <section className="w-full bg-[var(--background)]/95 px-5 py-6 sm:px-7 sm:py-8">
        <div className="mb-7 flex flex-col items-center text-center">
          <span className="mb-6 inline-flex items-center gap-1.5 rounded-full bg-[var(--brand-soft-lime)] px-4 py-2 text-xs font-medium text-[var(--foreground)]">
            أهلاً بك
          </span>
        ...
        <div className="w-full space-y-4">
          ...
          <Button
            ...
            onPress={() => {
              localStorage.setItem('auth_next', buildDevCallbackUrl(nextPath));
          ...
          <Button
            variant="outline"
            className="flex h-11 w-full items-center justify-center gap-2 rounded-full border border-[var(--border)] bg-[var(--background)] px-4 text-sm font-medium transition-all hover:border-[var(--border)] hover:bg-[var(--surface-secondary)]"
            onPress={() => {
              localStorage.setItem('auth_next', buildDevCallbackUrl(nextPath));
          ...
          >
            <FacebookIcon />
            تسجيل دخول سريع بـ Facebook
          </Button>

          <p className="pt-2 text-center text-xs leading-relaxed text-[var(--muted-foreground)]">
            بتسجيل الدخول عبر Google أو Facebook يُنشأ حسابك في منصة ركني إن لم
            يكن موجوداً مسبقاً.
          </p>
        </div>
      </section>
