...
const STORAGE_KEY = 'rukny-forms-sidebar-collapsed';
...
function SidebarAvatar({
  ...
  return (
    <div className="flex size-full items-center justify-center bg-[#1c1c1e] text-[13px] font-semibold text-white">
      {initials}
    </div>
  );
}
...
export function Sidebar({ avatarUrl, userName, userEmail }: SidebarProps) {
  ...
  const formsGroup = sidebarNavEntries.find(
    (entry): entry is Extract<typeof entry, { type: 'group' }> =>
      entry.type === 'group' && entry.id === 'forms',
  );
  ...
  const [formsOpen, setFormsOpen] = useState(() => {
    if (!formsGroup) return true;
    return formsGroup.defaultOpen === true || isNavGroupActive(pathname, formsGroup);
  });
  ...
  useEffect(() => {
    if (formsGroup && isNavGroupActive(pathname, formsGroup)) {
      setFormsOpen(true);
    }
  }, [pathname, formsGroup]);
  ...
  const formsHref = formsGroup?.children[0]?.href ?? `${APP_BASE}/forms`;
  const formsActive = formsGroup
    ? isNavGroupActive(pathname, formsGroup)
    : false;
                ...
                {sidebarNavEntries.map((entry) => {
                  ...
                  return (
                    <Link
                      key={entry.id}
                      href={formsHref}
                      className={collapsedIconClass(formsActive)}
                      aria-label={entry.label}
                      aria-current={formsActive ? 'page' : undefined}
                    >
                      <Icon size={19} strokeWidth={formsActive ? 2 : 1.7} />
          ...
          {collapsed ? (
            ...
            >
              ...
              >
                ...
                <Tooltip label={sidebarFooterItem.label} />
              </Link>
            </motion.div>
          ) : (
            <motion.div
