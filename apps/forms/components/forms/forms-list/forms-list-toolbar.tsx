...
function tabClass(active: boolean) {
  return cn(
    'min-h-10 flex-1 rounded-xl px-3 py-2 text-[13px] font-semibold transition-colors sm:min-h-0 sm:flex-none sm:px-3.5 sm:py-1.5 sm:text-[13px]',
    active
...
export function FormsListToolbar({
  ...
  return (
    <div className="flex flex-col gap-3 sm:gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div
        ...
        >
          {FILTER_OPTIONS.map((opt) => (
            <button
              ...
              className={cn(
                'min-h-9 shrink-0 rounded-full border px-3.5 py-2 text-[12px] font-semibold transition-colors sm:min-h-0 sm:rounded-xl sm:px-2.5 sm:py-1.5 sm:text-[12px]',
                status === opt.value
