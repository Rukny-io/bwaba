...
export function FormIntegrationsView({ formId }: { formId: string }) {
  ...
  if (error || !form) {
    return (
      <div className="rounded-2xl border border-[var(--danger)]/30 bg-[var(--danger)]/10 px-4 py-6 text-sm text-[var(--danger)]">
  ...
  if (selected) {
    return (
      <div className="space-y-5">
        <div className="dashboard-panel shadow-sm shadow-black/[0.02] sm:rounded-3xl sm:p-6">
          <button
            type="button"
            onClick={() => {
              setSelected(null);
  ...
  return (
    <div className="space-y-5 sm:space-y-6">
      ...
      {sheetsOAuthError ? (
        <div
          className="rounded-2xl border border-[var(--danger)]/30 bg-[var(--danger)]/8 px-4 py-3 text-sm text-[var(--danger)]"
          role="alert"
        >
          {sheetsOAuthError}
          <button
            ...
            onClick={() => setSheetsOAuthError(null)}
          >
            إغلاق
          </button>
        </div>
      ...
      {connectedCount === 0 ? (
        <section className="rounded-2xl border border-dashed border-[var(--border)] bg-[var(--surface-secondary)]/20 px-4 py-5 text-center sm:rounded-3xl">
          <p className="text-sm font-semibold text-[var(--foreground)]">
            لم تربط أي تكامل بعد
          </p>
          <p className="mt-1 text-xs text-[var(--muted-foreground)]">
            ابدأ بـ Google Sheets أو Webhook
          </p>
        </section>
      ) : null}
