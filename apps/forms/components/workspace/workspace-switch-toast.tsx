...
export function WorkspaceSwitchToast() {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className="fixed bottom-4 start-1/2 z-50 flex -translate-x-1/2 items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-sm shadow-lg"
    >
      <span className="flex size-5 items-center justify-center rounded-full bg-emerald-500 text-white">
        <Check className="size-3" />
      </span>
      <span>
        تم التبديل إلى مساحة <span className="font-semibold">{label}</span>
      </span>
    </div>
  );
}
