/**
 * API client — cookies + CSRF + refresh via Forms BFF
 */
...
const REFRESH_STATE_KEY = '__forms_refresh_state__';
...
async function apiClient<T>(
  ...
  const activeWorkspaceId = readActiveWorkspaceIdFromBrowser();
  if (activeWorkspaceId && !headers[ACTIVE_WORKSPACE_HEADER]) {
...
export const api = {
  ...
  delete: <T>(endpoint: string, options?: { data?: unknown }) =>
    apiClient<T>(endpoint, { method: 'DELETE', body: options?.data }),
};
