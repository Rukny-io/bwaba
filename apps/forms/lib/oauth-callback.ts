...
export function readOAuthCallbackParams(
  ...
  // Prefer whatever looks like a fresh Redis handoff (64–128 hex). If both are
  // present and differ, prefer the fragment — Google historically delivered there.
  ...
  // Fall back to any non-empty value (validation happens server-side).
  if (queryCode) {
    return { code: queryCode, next: queryNext || hash.next };
  }
