...
export function parseMailQuery(
...
): Required<Pick<MailAppsListQuery, "page" | "limit" | "tab">> &
  MailAppsListQuery {
  ...
  const limit = Math.min(
    ...
    Math.max(
      1,
      Number(params.get("limit") || String(MAIL_DEFAULT_LIMIT)) ||
        MAIL_DEFAULT_LIMIT,
    ),
  );
  ...
  return {
    ...
    domainStatus:
      (params.get("domainStatus") as MailAppsListQuery["domainStatus"]) ||
      undefined,
  };
}
...
export function buildMailSearchParams(
  ...
  setOrDelete(
    "tab",
    query.tab && query.tab !== "review" ? query.tab : undefined,
  );
