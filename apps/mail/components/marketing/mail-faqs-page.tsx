  ...
  return (
    <main className="overflow-x-clip">
      ...
      <section className={`${L.container} py-12 sm:py-16`}>
        <div className="overflow-hidden border border-[#e7e5e4] bg-white">
          {MAIL_FAQS.map((item) => (
            <details
              key={item.id}
              className="group border-b border-[#e7e5e4] last:border-b-0"
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-6 px-5 py-5 text-left marker:content-none sm:px-6 sm:py-6 [&::-webkit-details-marker]:hidden">
                <span className="text-[1.05rem] font-semibold tracking-tight text-[#1c1917] sm:text-xl">
                  {item.question}
                </span>
                <ChevronDown
                  className="size-5 shrink-0 text-[#a8a29e] transition-transform group-open:rotate-180"
                  aria-hidden
                />
              </summary>
              <p className="max-w-2xl px-5 pb-6 text-[15px] leading-[1.8] text-[#57534e] sm:px-6">
                {item.answer}
              </p>
            </details>
          ))}
      ...
      <section className="border-t border-[#e7e5e4] pb-12 sm:pb-16 md:pb-[72px]">
        <div className={L.container}>
          <div className="border border-[#e7e5e4] bg-white px-6 py-12 text-center sm:px-12">
            <p className="max-w-sm mx-auto text-[15px] text-[#57534e]">
