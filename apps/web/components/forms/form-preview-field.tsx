...
export function FormPreviewField({
  ...
  if (type === 'SIGNATURE') {
    return (
      <div className="public-form-field space-y-2.5">
        ...
        <div
          className={cn(
            'flex h-28 items-center justify-center rounded-xl border border-dashed text-xs',
            themed
              ? 'border-[var(--form-input-border)] bg-[color-mix(in_srgb,var(--form-input-border)_18%,var(--form-card))] text-[var(--form-text-body)]'
              : 'border-[var(--border)] bg-[var(--surface-secondary)]/50 text-[var(--muted-foreground)]',
          )}
        >
          منطقة التوقيع — قريباً
        </div>
      </div>
    );
  }
  ...
  return (
    <div className={questionWrapClass(themed)}>
      ...
      />
      {fieldError ? (
        <p className="public-form-field-error mt-1.5 text-xs" role="alert">
          {fieldError}
        </p>
      ) : null}
    </div>
